...
__wt_desc_write(WT_SESSION_IMPL *session, WT_FH *fh, uint32_t allocsize)
{
	...
	ret = pm_buf_write_with_flusher(conn->pmw, fh->name, fh->name_hash, (off_t)0, desc->checksum, (size_t)allocsize, (byte*)desc);
	//ret = pm_buf_write_with_flusher_append(conn->pmw, fh->name, fh->name_hash, (off_t)0, desc->checksum, (size_t)allocsize, (byte*)desc);
...
__desc_read(WT_SESSION_IMPL *session, WT_BLOCK *block)
{
	...
	const PMEM_BUF_BLOCK* pblock =
		pm_buf_read(conn->pmw, block->fh->name, block->fh->name_hash, (off_t)0, 0, (size_t)block->allocsize, buf->mem);
		//pm_buf_read_append(conn->pmw, block->fh->name, block->fh->name_hash, (off_t)0, (size_t)block->allocsize, buf->mem);
